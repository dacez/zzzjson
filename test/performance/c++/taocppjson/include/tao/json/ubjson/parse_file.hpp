   ...
   namespace json
   {
      namespace ubjson
      {
         ...
         basic_value< Traits, Base > basic_parse_file( const std::string& filename )
         {
            ...
            ubjson::events::parse_file( consumer, filename );
         ...
         {
            return ubjson::basic_parse_file< Traits, json::internal::empty_base, Transformers... >( filename );
         }

         template< template< typename... > class... Transformers >
         value parse_file( const std::string& filename )
         {
            return ubjson::basic_parse_file< traits, Transformers... >( filename );
         }
