...
#ifndef TAO_JSON_MSGPACK_PARSE_FILE_HPP
#define TAO_JSON_MSGPACK_PARSE_FILE_HPP
   ...
   namespace json
   {
      namespace msgpack
      {
         ...
         basic_value< Traits, Base > basic_parse_file( const std::string& filename )
         {
            ...
            msgpack::events::parse_file( consumer, filename );
         ...
         {
            return msgpack::basic_parse_file< Traits, json::internal::empty_base, Transformers... >( filename );
         }

         template< template< typename... > class... Transformers >
         value parse_file( const std::string& filename )
         {
            return msgpack::basic_parse_file< traits, Transformers... >( filename );
         }
