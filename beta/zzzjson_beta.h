...
static inline void *z_New(z_SIZE size) { return malloc(size); }
...
static inline z_BOOL z_ValueParseLen(struct z_Value *v, const char *s,
                                     z_SIZE len);
...
static inline const char *z_ValueGetStringFast(const struct z_Value *v,
                                               z_SIZE *len);
...
static inline const char *z_ValueGetNumFast(const struct z_Value *v,
                                            z_SIZE *len);
...
static inline const char *z_ValueGetKeyFast(const struct z_Value *v,
                                            z_SIZE *len);

static inline struct z_Value *z_ValueObjGet(const struct z_Value *v,
                                            const char *key);
static inline struct z_Value *z_ValueObjGetLen(const struct z_Value *v,
                                               const char *key, z_SIZE len);

static inline const z_JSONType *z_ValueType(const struct z_Value *v);

static inline z_SIZE z_ValueSize(const struct z_Value *v);

static inline struct z_Value *z_ValueArrayGet(const struct z_Value *v,
                                              z_SIZE index);
...
static inline z_BOOL z_ValueSetNumStrLenFast(struct z_Value *v, const char *num,
                                             z_SIZE len);
static inline z_BOOL z_ValueSetNumStr(struct z_Value *v, const char *num);
static inline z_BOOL z_ValueSetNumStrLen(struct z_Value *v, const char *num,
                                         z_SIZE len);
...
static inline z_BOOL z_ValueSetStrLenFast(struct z_Value *v, const char *str,
                                          z_SIZE len);
static inline z_BOOL z_ValueSetStr(struct z_Value *v, const char *str);
static inline z_BOOL z_ValueSetStrLen(struct z_Value *v, const char *str,
                                      z_SIZE len);

static inline z_BOOL z_ValueSetKeyFast(struct z_Value *v, const char *key);
static inline z_BOOL z_ValueSetKeyLenFast(struct z_Value *v, const char *key,
                                          z_SIZE len);
static inline z_BOOL z_ValueSetKey(struct z_Value *v, const char *key);
static inline z_BOOL z_ValueSetKeyLen(struct z_Value *v, const char *key,
                                      z_SIZE len);
...
static inline z_BOOL z_ValueArrayAdd(struct z_Value *v,
                                     const struct z_Value *vv);
...
// 短命名API，详见《API》
static inline Allocator *NewAllocator() { return z_AllocatorNew(); }
static inline void ReleaseAllocator(Allocator *root_alloc) {
  z_AllocatorRelease(root_alloc);
}
static inline Value *NewValue(Allocator *alloc) { return z_ValueNew(alloc); }
...
static inline BOOL Parse(Value *v, const char *s) { return z_ValueParse(v, s); }
...
static inline const char *GetString(Value *v) { return z_ValueGetString(v); }
static inline const char *GetNumFast(const Value *v, z_SIZE *len) {
  return z_ValueGetNumFast(v, len);
}
static inline const char *GetNumStr(Value *v) { return z_ValueGetNumStr(v); }
static inline const double *GetNum(Value *v) { return z_ValueGetNum(v); }
static inline const BOOL *GetBool(const Value *v) { return z_ValueGetBool(v); }
static inline BOOL IsNull(const Value *v) { return z_ValueIsNull(v); }
static inline const char *GetKey(Value *v) { return z_ValueGetKey(v); }
...
static inline const JSONType *Type(const Value *v) { return z_ValueType(v); }
static inline SIZE Size(const Value *v) { return z_ValueSize(v); }
static inline Value *ArrayGet(const Value *v, SIZE index) {
  return z_ValueArrayGet(v, index);
}
static inline Value *Begin(const Value *v) { return z_ValueBegin(v); }
static inline Value *Next(const Value *v) { return z_ValueNext(v); }
static inline Value *Copy(const Value *v) { return z_ValueCopy(v); }
static inline BOOL Move(Value *v) { return z_ValueMove(v); }
static inline void SetNull(Value *v) { z_ValueSetNull(v); }
static inline void SetBool(Value *v, BOOL b) { z_ValueSetBool(v, b); }
...
static inline void SetArray(Value *v) { z_ValueSetArray(v); }
static inline void SetObj(Value *v) { z_ValueSetObj(v); }
static inline BOOL SetFast(Value *v, Value *vv) {
  return z_ValueSetFast(v, vv);
}
static inline BOOL Set(Value *v, const Value *vv) { return z_ValueSet(v, vv); }
...
// 字符串长度计算函数
static inline z_SIZE z_StrLen(const char *str) { return (z_SIZE)strlen(str); }

static inline double z_StrToDouble(const char *str) { return atof(str); }
...
// 字符串比较，len为b的长度。
static inline z_BOOL z_StrIsEqualLen(const char *a, z_SIZE a_len, const char *b,
                                     z_SIZE b_len) {
...
static inline struct z_Allocator *z_AllocatorNew() {
  // 每次分配一大块内存，避免多次分配
  void *ptr = z_New(sizeof(struct z_Allocator) + sizeof(struct z_ANode) +
                    z_AllocatorInitMemSize);
  ...
  alloc->Root->Data =
      (char *)ptr + sizeof(struct z_Allocator) + sizeof(struct z_ANode);
  alloc->Root->Pos = 0;
  alloc->Root->Next = 0;
  return alloc;
}
...
static inline void z_AllocatorAppendChild(z_SIZE init_size,
                                          struct z_Allocator *alloc) {
  // 每次分配一大块内存，避免多次分配
...
// 字符串
// 字符串由内存分配器分配内存，当追加操作导致内存不够时，直接分配 z_Delta
// 倍内存，并把旧内存拷贝到新内存中
struct z_String {
  // 数据
  char *Data;
...
// 新建一个字符串
static inline struct z_String *z_StringNew(struct z_Allocator *alloc,
                                           z_SIZE init_size) {
  struct z_String *str = (struct z_String *)z_AllocatorAlloc(
      alloc, sizeof(struct z_String) + init_size);
  str->Size = init_size;
...
// 清空一个字符串
static inline void z_StringReset(struct z_String *str) { str->Pos = 0; }

// 追加字符串
static inline void z_StringAppendStr(struct z_String *str, const char *s,
                                     z_SIZE size) {
...
static inline struct z_Value *z_ValueNew(struct z_Allocator *alloc) {
  struct z_Value *v =
      (struct z_Value *)z_AllocatorAlloc(alloc, sizeof(struct z_Value));
  v->A = alloc;
  v->N = 0;
  z_ValueInitCache(v);
  return v;
...
// 创建一个值，并赋值
static inline struct z_Value *z_ValueInnerNew(struct z_Allocator *alloc,
                                              struct z_Node *n) {
  struct z_Value *v =
      (struct z_Value *)z_AllocatorAlloc(alloc, sizeof(struct z_Value));
  v->A = alloc;
...
static inline z_BOOL z_Skin(const char c) {
  if (z_UNLIKELY(z_UNLIKELY(c == ' ') || z_UNLIKELY(c == '\t') ||
                 z_UNLIKELY(c == '\n') || z_UNLIKELY(c == '\r'))) {
    return z_True;
  }
  return z_False;
...
// 预期消费一个字符成功
static inline z_BOOL z_LikelyConsume(const char c, const char *s,
                                     z_SIZE *index) {
...
// 预期消费一个字符失败
static inline z_BOOL z_UnLikelyConsume(const char c, const char *s,
                                       z_SIZE *index) {
...
// 预期消费下一个有效字符成功
static inline z_BOOL z_LikelyPeekAndConsume(const char c, const char *s,
                                            z_SIZE *index) {
  while (z_UNLIKELY(z_Skin(s[*index])))
    ++(*index);
  if (z_LIKELY(s[*index] == c)) {
...
// 预期消费下一个有效字符失败
static inline z_BOOL z_UnLikelyPeekAndConsume(const char c, const char *s,
                                              z_SIZE *index) {
  while (z_UNLIKELY(z_Skin(s[*index])))
    ++(*index);
  if (z_UNLIKELY(s[*index] == c)) {
...
// 专为 z_ValueGetUnEscapeString 使用
static inline void z_ConsumeHexOneForUnEscape(const char *s, z_SIZE *index,
                                              z_SIZE *cp) {
  *cp = *cp << 4;
...
static inline void z_ConsumeHexForUnEscape(const char *s, z_SIZE *index,
                                           z_SIZE *cp) {
      ...
      case 'u': {
        ...
        if (z_LIKELY(z_ConsumeHex(s, index, &cp))) {
          ...
          if (z_UNLIKELY(cp >= 0xDC00 && cp <= 0xDFFF))
            return z_False;
          if (z_UNLIKELY(cp >= 0xD800 && cp <= 0xDBFF)) {
            if (z_LIKELY(z_LikelyConsume('\\', s, index) &&
                         z_LikelyConsume('u', s, index))) {
              z_SIZE cp2 = 0;
...
static inline z_BOOL z_CheckStr(const char *s, z_SIZE *len) {
  ...
  c = s[index++];
  while (z_LIKELY(c != 0)) {
    if (z_UNLIKELY(z_UNLIKELY((unsigned char)c <= 0x1f) ||
                   z_UNLIKELY(c == '"')))
      return z_False;
    if (z_UNLIKELY(c == '\\')) {
      c = s[index++];
...
static inline z_BOOL z_CheckStrLen(struct z_Allocator *alloc, const char *s,
                                   z_SIZE len) {
  ...
  z_SIZE avail_len;
  if (z_UNLIKELY(z_CheckStr(z_StringStr(z_StringCache), &avail_len) ==
                 z_False)) {
    return z_False;
  }
  if (z_UNLIKELY(avail_len != len))
    return z_False;
  return z_True;
}
...
static inline z_BOOL z_ConsumeNum(const char *s, z_SIZE *index) {
  ...
  } else if (z_LIKELY(z_LIKELY(s[*index] >= '1') &&
                      z_LIKELY(s[*index] <= '9'))) {
    char c = s[++(*index)];
...
static inline z_BOOL z_CheckNumLen(struct z_Allocator *alloc, const char *s,
                                   z_SIZE len) {
  if (z_UNLIKELY(z_StringCache == 0)) {
    z_StringCache = z_StringNew(alloc, z_StringCacheInitMemSize);
  } else {
  ...
  z_StringAppendEnd(z_StringCache);
  z_SIZE avail_len;
  if (z_UNLIKELY(z_CheckNum(z_StringStr(z_StringCache), &avail_len) ==
                 z_False)) {
    return z_False;
  }
  if (z_UNLIKELY(avail_len != len))
...
static inline z_BOOL z_ValueParseFast(struct z_Value *v, const char *s) {
  ...
  case '[': {
    ...
    struct z_Node *n =
        (struct z_Node *)z_AllocatorAlloc(v->A, sizeof(struct z_Node));
    n->Father = node;
    n->Prev = 0;
  ...
  while (z_LIKELY(node != v->N)) {
    ...
    while (z_LIKELY(node != v->N)) {
      if (z_LikelyPeekAndConsume(',', s, &index)) {
        ...
        char c = z_Peek(s, &index);
        if (z_LIKELY(
                (c == '}' && z_LIKELY(node->Father->Type == z_JSONTYPEOBJ)) ||
                z_LIKELY(z_LIKELY(c == ']') &&
                         z_LIKELY(node->Father->Type == z_JSONTYPEARRAY)))) {
          node->Next = 0;
          node = node->Father;
        } else {
          if (z_LIKELY(src_node == 0))
...
static inline struct z_Value *z_ValueObjGetLen(const struct z_Value *v,
  ...
  struct z_Node *next = v->N->Value.Node;
  while (z_LIKELY(next != 0)) {
    if (z_UNLIKELY(z_StrIsEqualLen(key, len, next->Key, next->KeyLen) ==
                   z_True)) {
      struct z_Value *ret_val = z_ValueInnerNew(v->A, next);
      return ret_val;
    }
  ...
  do {
    ...
    case z_JSONTYPEARRAY:
      ...
      if (z_LIKELY(node->Value.Node != 0)) {
        node = node->Value.Node;
        struct z_Node *n =
            (struct z_Node *)z_AllocatorAlloc(A, sizeof(struct z_Node));
        n->Father = des_node;
        n->Prev = 0;
...
static inline z_BOOL z_ValueSetKeyFast(struct z_Value *v, const char *key) {
  ...
  if (z_UNLIKELY(v->N == 0)) {
    ...
    v->N->Len = 4;
             ...
             z_UNLIKELY(v->N->Father->Type != z_JSONTYPEOBJ)) {
    return z_False;
  }
  z_ValueInitCache(v);
  v->N->Key = key;
...
// 函数说明详见《API》
static inline z_BOOL z_ValueObjAdd(struct z_Value *v,
                                   const struct z_Value *vv) {
  if (z_UNLIKELY(v->N == 0))
    return z_False;
  if (z_UNLIKELY(v->N->Type != z_JSONTYPEOBJ))
...
// 函数说明详见《API》
static inline z_BOOL z_ValueArrayAddFast(struct z_Value *v,
                                         struct z_Value *vv) {
  if (z_UNLIKELY(v->N == 0))
    return z_False;
  if (z_UNLIKELY(v->N->Type != z_JSONTYPEARRAY))
